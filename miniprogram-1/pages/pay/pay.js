...
Page({
  ...
  getUserPoints() {
    ...
    const balance = memberInfo.balance || 0;
    // 假设最大抵扣金额为订单金额的一半
    ...
    this.setData({
      userPoints: points,
      balance: balance,
      balanceRmb: Number(balance).toFixed(2),
...
wx.request({
  ...
  success: (res) => {
    if (res.data.code === 200 || res.data.code === 201) {
      ...
      // 获取后端返回的返现金额
      const cashback = Number(res.data.cashback || 0);
